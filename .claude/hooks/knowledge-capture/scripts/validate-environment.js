...
// Check 4: Qdrant availability
function checkQdrant() {
  const http = require('http');

  return new Promise((resolve) => {
    const req = http.get('http://localhost:6333/collections', { timeout: 3000 }, (res) => {
      if (res.statusCode === 200) {
        log.ok('Qdrant available');
      } else {
        warnings.push({
          check: 'Qdrant',
          current: `HTTP ${res.statusCode}`,
          fix: 'Run: docker-compose up -d'
        });
        log.warn('Qdrant returned unexpected status');
      }
      resolve();
    });
    ...
    req.on('error', () => {
      ...
      log.warn('Qdrant not available (run docker-compose up -d)');
      resolve();
    });
